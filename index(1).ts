...
- Return valid JSON only.`;
    ...
    const {data:campaign,error:campaignError}=await supabase.from("campaigns").insert({name:campaignName,product_id:product.id,platform:body.platform||"TikTok",target_audience:body.target_audience||null,tone:body.tone||null,status:"Active",total_clips:count,big_idea:generated.big_idea||null,strategy_summary:generated.strategy_summary||null,quality_score:Number(generated.quality_score||0),generation_source:"Gemini",generation_settings:{goal:body.goal,platform:body.platform,tone:body.tone,creator:body.creator,creativity:body.creativity,clip_count:count}}).select("id").single();
    ...
    await supabase.from("creative_runs").insert({campaign_id:campaign.id,product_id:product.id,model,status:"completed",clip_count:count,settings:{goal:body.goal,platform:body.platform,tone:body.tone,creator:body.creator,creativity:body.creativity}});
